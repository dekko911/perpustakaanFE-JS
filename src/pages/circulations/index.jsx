...
import { api_token, api_url, parseTimestampToDateOnly } from "../../lib/utils";
...
export const CirculationsPage = () => {
	...
	useEffect(() => {
		const fetchCirculations = async () => {
			const res = await axios.get(`${api_url}/api/circulations`, {
				headers: {
					...
					<TableBody>
						{circulations.map((circulation, i) => (
							<TableRow key={i}>
								...
								<TBodyCell>{parseTimestampToDateOnly(circulation.tanggal_pinjam)}</TBodyCell>
								<TBodyCell>{parseTimestampToDateOnly(circulation.jatuh_tempo)}</TBodyCell>
