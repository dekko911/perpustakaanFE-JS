...
export const InputCreate = ({
	...
	return (
		<input
			className={cn(
				"w-full p-2 border-1 border-zinc-500 rounded-md text-white focus:outline-2 focus:outline-zinc-200/20 mt-1.5 mb-2",
				className
			)}
			type={type}
			name={name}
...
export const InputCreateFile = ({ name = "", className = "", onChange }) => {
	return (
		<input
			...
			name={name}
			className={cn(
				"block border-1 border-zinc-500 w-full p-2 rounded-md mt-1 mb-2 cursor-pointer",
				className
			)}
			onChange={onChange}
		/>
